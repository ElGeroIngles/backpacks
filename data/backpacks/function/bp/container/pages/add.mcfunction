...
# Add items:
execute unless score $first_pages backpacks.pages = @s backpacks.pages run item replace entity @n[tag=backpacks.backpack_menu_page_placeholder] container.18 with arrow[custom_name='{"text":"Go Back","color":"green","italic":false}',custom_data={menu_backpack:{go_back:1b}}]
execute if score @s backpacks.pages matches 2.. run item replace entity @n[tag=backpacks.backpack_menu_page_placeholder] container.26 with arrow[custom_name='{"text":"Next Page","color":"green","italic":false}',custom_data={menu_backpack:{next_page:1b}}]
